...
void GraphicsViewGame::monthMapOnTheScreen()
{
    ...
    for (auto i = 0; i < tmxMap->layers.size(); ++i) {
        auto layer = tmxMap->layers[i];

        if (i == 3) {
            actors = new ActorGraphicsItem*[2];

            actors[0] = new NinjaGraphicItem();
...
void GraphicsViewGame::widCompiler()
{
    QPlainTextEdit *planTextCompiler = new QPlainTextEdit(this);

    int w = 300;
    int h = 100;

    planTextCompiler->setGeometry(0, height(), w, h);
    planTextCompiler->setWindowOpacity(0.1);
}
...
bool GraphicsViewGame::actionPerformed(int key, int actor)
{
    ...
    qDebug() << (actor == ActorExternal? "EXTERNO" : "");
